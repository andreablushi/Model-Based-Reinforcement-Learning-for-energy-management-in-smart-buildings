...
class PreTrainedXGBoost:
    def __init__(self, steps_ahead, variable_name):
        ...
        for i in range(steps_ahead):
            path = f'agents/checa/forecast_agent/pretrained/xgboost_{variable_name}_{i}_1.json'
            model1 = XGBRegressor()
            model1.load_model(path)
            path = f'agents/checa/forecast_agent/pretrained/xgboost_{variable_name}_{i}_2.json'
            model2 = XGBRegressor()
